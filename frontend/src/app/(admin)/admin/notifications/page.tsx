...
function Stat({ label, value, icon: Icon, color }: { label: string; value: number; icon: any; color: string }) {
  return (
    <div style={{ background: 'var(--surface-1)', border: '1px solid var(--line-soft)', borderRadius: '10px', padding: '1rem' }}>
      ...
      <p style={{ fontFamily: 'var(--f-mono)', fontSize: '2rem', fontWeight: 700, color: 'var(--text-primary)', lineHeight: 1 }}>{value}</p>
    </div>
  );
}
    ...
    <AdminShell>
      <div className="admin-page">
        <div style={{ marginBottom: '1.5rem' }}>
          ...
          <h1 style={{ fontSize: '1.5rem', fontWeight: 700, color: 'var(--text-primary)' }}>Системные уведомления</h1>
        </div>
        ...
        {loading && (
          <>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '1rem', marginBottom: '1.5rem' }}>
              {[1, 2, 3].map((i) => (
                <div key={i} style={{ background: 'var(--surface-1)', border: '1px solid var(--line-soft)', borderRadius: '10px', padding: '1rem' }}>
                  <Skeleton h={12} w={120} />
        ...
        {!loading && error && (
          <div style={{ borderLeft: '4px solid var(--accent-crimson)', background: 'var(--surface-1)', borderRadius: '10px', padding: '0.875rem 1rem', color: 'var(--text-primary)' }}>
            ...
            <p style={{ fontSize: '0.875rem', color: 'var(--text-secondary)', marginBottom: '0.75rem' }}>{error}</p>
            <button onClick={load} style={{ background: 'var(--accent-indigo)', color: '#fff', border: 'none', borderRadius: '8px', padding: '9px 16px', fontWeight: 600, cursor: 'pointer' }}>
              Попробовать снова
            </button>
          </div>
        )}
        ...
        {!loading && data && (
          <>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '1rem', marginBottom: '1.5rem' }}>
              <Stat label="Новые заявки" value={data.counts.newApplications} icon={FileText} color="var(--accent-indigo)" />
              <Stat label="Просроченные" value={data.counts.overdueLoans} icon={AlertTriangle} color="#C08020" />
              <Stat label="Заявки на оплату" value={data.counts.pendingPayments} icon={Wallet} color="var(--accent-mint)" />
            </div>

            <div style={{ background: 'var(--surface-1)', border: '1px solid var(--line-soft)', borderRadius: '10px', padding: '1rem' }}>
              <h2 style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', fontSize: '1.0625rem', fontWeight: 700, color: 'var(--text-primary)', marginBottom: '1rem' }}>
                <Bell size={18} color="var(--accent-indigo)" /> События
              </h2>
              {data.recent.length === 0 ? (
                <div style={{ textAlign: 'center', padding: '2.5rem 1rem' }}>
                  <Bell size={60} color="var(--text-secondary)" style={{ margin: '0 auto 1rem', display: 'block' }} />
                  <h3 style={{ fontSize: '1.125rem', fontWeight: 700, color: 'var(--text-primary)', marginBottom: '0.5rem' }}>Событий пока нет</h3>
                  <p style={{ color: 'var(--text-secondary)' }}>Новые заявки, просрочки и заявки на оплату появятся здесь.</p>
                </div>
              ) : (
                  ...
                  {data.recent.map((n) => {
                    const body = (
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', padding: '0.75rem 0', borderBottom: '1px solid #F0F3F6', gap: '1rem' }}>
                        <div style={{ flex: 1 }}>
                          <p style={{ fontWeight: 600, color: 'var(--text-primary)', fontSize: '0.9375rem' }}>{n.title}</p>
                          <p style={{ color: 'var(--text-secondary)', fontSize: '0.8125rem', marginTop: '2px' }}>{n.body}</p>
                        </div>
                        <p style={{ color: 'var(--text-secondary)', fontSize: '0.75rem', whiteSpace: 'nowrap' }}>{formatDate(n.createdAt)}</p>
                      </div>
                    );
