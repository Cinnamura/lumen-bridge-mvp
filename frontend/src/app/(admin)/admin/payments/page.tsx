          ...
          <div style={{ background: '#fff', border: '1px solid #E8ECF0', borderRadius: '12px', overflow: 'hidden' }}>
            <div style={{ overflowX: 'auto' }}>
              <table className="admin-table">
                ...
                <tbody>
                  {rows.length === 0 && <tr><td colSpan={7} style={{ textAlign: 'center', padding: '2.5rem', color: '#4A6580' }}>Заявок не найдено</td></tr>}
                  {rows.map((p) => {
                    ...
                    return (
                      <tr key={p.id}>
                        ...
                        <td><Link href={`/admin/loans/${p.loanId}`} style={{ fontSize: '0.8125rem', color: '#2E7DF7', textDecoration: 'none', fontWeight: 600 }}>Открыть займ</Link></td>
                        <td className="col-actions">
                          {p.status === 'pending' ? (
                            <div style={{ display: 'inline-flex', gap: '0.5rem', flexWrap: 'wrap' }}>
                              <button onClick={() => decide(p.id, 'confirmed')} disabled={updatingId === p.id} style={{ background: '#1E8A5E', color: '#fff', border: 'none', borderRadius: '6px', padding: '6px 12px', fontSize: '0.8125rem', fontWeight: 600, cursor: 'pointer' }}>
                                Подтвердить
                              </button>
                              <button onClick={() => decide(p.id, 'rejected')} disabled={updatingId === p.id} style={{ background: '#fff', color: '#C0392B', border: '1.5px solid #C0392B', borderRadius: '6px', padding: '6px 12px', fontSize: '0.8125rem', fontWeight: 600, cursor: 'pointer' }}>
                                Отклонить
                              </button>
                            </div>
                          ) : (
